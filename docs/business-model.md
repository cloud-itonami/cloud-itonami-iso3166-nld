# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — the Netherlands

## Classification

- Repository: `cloud-itonami-iso3166-nld`
- ISO 3166: `NLD` (the Netherlands)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on a Dutch
  public contract
...
- registration walkthrough for TenderNed, the Netherlands' official public procurement platform
- business/tax registration checklist: a KVK (Kamer van Koophandel, Chamber of Commerce) business registration; BTW (VAT) registration with the Belastingdienst where the taxable-turnover threshold is met
- local-content / preferential-procurement navigation: EU-wide open tendering above EU thresholds (no national-content quota as an EU member state), but Dutch-language submission and a domestic/EU address for service are typically required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Dutch-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in the Netherlands).

...
2. Register the client's intake: business type, target public function,
   prior filing history in the Netherlands if any.
3. Run the advisor in read-only mode against TenderNed, the Netherlands' official public procurement platform.
...
- named referral relationship with Dutch-licensed counsel or a
...
a working referral relationship with Dutch-licensed counsel or a
registered agent for whatever licensed representation the law of
the Netherlands requires for actual public-procurement filings.

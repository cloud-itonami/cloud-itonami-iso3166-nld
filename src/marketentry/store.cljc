(ns marketentry.store
  "SSoT for the NLD market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems NL" :portal "TenderNed"
             ...
             :requires-kvk? true :kvk-verified? true
             :drafted? false :submitted? false
             :jurisdiction "NLD" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "TenderNed"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-kvk? true :kvk-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "TenderNed"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "NLD" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "TenderNed"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "NLD" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "TenderNed"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-kvk? true :kvk-verified? false
             :drafted? false :submitted? false
             :jurisdiction "NLD" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "TenderNed"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-kvk?)    (assoc :engagement/requires-kvk? requires-kvk?)
    (some? kvk-verified?)    (assoc :engagement/kvk-verified? kvk-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-kvk? :engagement/kvk-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-kvk? (boolean (:engagement/requires-kvk? m))
     :kvk-verified? (boolean (:engagement/kvk-verified? m))

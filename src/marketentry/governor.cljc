(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Netherlands-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Netherlands-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Netherlands
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(KVK/統一資格/TenderNed登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Netherlands public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はオランダ居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- kvk-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-kvk? true`, INDEPENDENTLY check
  `:kvk-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-kvk? e))
                 (not (true? (:kvk-verified? e))))
        [{:rule :kvk-unverified
          :detail (str subject " はKVK確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (kvk-unverified-violations request st)
